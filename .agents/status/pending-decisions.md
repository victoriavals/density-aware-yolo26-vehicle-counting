...
| Kode | Isi | Status |
|---|---|---|
| **A-01** (=K4) | Redaksi/alternatif abstrak bila hasil tidak signifikan | ✅ **SELESAI 6 Agu 2026** — abstrak ID+EN ditulis ulang & DITEMPEL ke `TESIS_BAB1-3_REVISI_SIDANG_v8.docx` (115 & 140 kata; total 336 & 385, setara v7). Riwayat: **TERPICU — usulan lengkap SIAP di `hasil_bab4_5/K4_REDAKSI_HASIL.md`** (definisi "konfigurasi terbaik", aturan pelaporan 3 hipotesis, abstrak ditulis ulang ID+EN siap tempel, daftar frasa terlarang). Tinggal ACC pembimbing. Angka final pasca-FASE 1: H1 (V8−V1) p=**0,565**, H2 (V4−V1) p=**0,208** r=−0,300, H3 (V8−V5) p=**0,037** r=+0,487 SIGNIFIKAN. ⚠️ Kalimat hasil abstrak v7 **memprasyaratkan** kenaikan signifikan atas baseline yang TIDAK didukung: mAP@0,5 V8 77,97 % < V1 **78,61 %** (turun 0,64 poin); mAP@0,5:0,95 hanya +0,19 poin. Abstrak harus **DITULIS ULANG**, bukan diisi. Rincian: `hasil_bab4_5/PETA_PLACEHOLDER_ABSTRAK.md`. Keputusan Naufal + pembimbing. |
...
| **B-01** | Cek batas kata abstrak (±360) | ✅ **SELESAI** — ABSTRAK v8 **336 kata** (v7 337), ABSTRACT **385** (v7 380). Draf pertama sempat 451/505 kata → dipadatkan. |
...
| **K7b** | Penghitung kedua | ✅ **DISELESAIKAN LEWAT REVISI NASKAH (6 Agu 2026)** — Subbab 3.10.1 v8 kini menyatakan **satu** penghitung + keterbatasan eksplisit, sehingga BAB 3 dan BAB 4 konsisten. Bila penghitung kedua kelak tersedia, kit tetap siap. Riwayat: kit SIAP PAKAI — `video_uji/penghitung_kedua/` berisi template buta klip 4 (`gt_4_vidiouji_B.csv`), salinan A, dan pratinjau garis; jalur `bandingkan_gt.py` sudah diuji. Protokol + cadangan *test-retest* + redaksi revisi 3.10.1: `hasil_bab4_5/K7_PENGHITUNG_KEDUA.md`. ⚠️ Bila A≠B, GT hasil kesepakatan menjadi final dan **MAE/RMSE/MAPE harus dihitung ulang**. |
...
| **Label GPU** | ✅ **SELESAI 6 Agu 2026** | v8 memakai "RTX 4060 **Ti** 8GB" di **6 lokasi** (5 paragraf + 1 sel Tabel 3.9). |
